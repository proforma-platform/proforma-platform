  ...
  if (session?.current_mission_id && session?.current_trace_id && session?.current_run_id) {
    ...
    try {
      ...
      if (fs.existsSync(blockedFile)) {
        const message = String(fs.readFileSync(blockedFile, "utf8") || "").trim() || "Erro sem detalhe informado. Aguardando tratamento para continuar a execução.";
      ...
      if (fs.existsSync(resumeFile)) {
        const message = String(fs.readFileSync(resumeFile, "utf8") || "").trim() || "Bloqueio tratado. Execução retomada.";
        await post("/api/govhub/operations/execution-events", {
          session_id: sessionId,
          session_token: sessionToken,
          mission_id: session.current_mission_id,
      ...
      if (fs.existsSync(doneFile)) {
        const rawDone = String(fs.readFileSync(doneFile, "utf8") || "").trim();
